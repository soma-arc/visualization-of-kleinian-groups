...
ここでは，クライン群の
\emph{群}(\textit{Group})という語は変換の集まりだと考えるとわかりやすい.
...
この合成変換はどの点に作用させても変換$a$の\emph{固定点}に収束する．
...
原点を中心とする回転について考える.
この矩形を中心として考えたいとする.
原点を中心とした回転を$r$, 矩形の中心までの平行移動を$t$とすると, 矩形の
中心における回転を表す変換を以下のように計算することができる.
\begin{equation*}
 \hat{r} = trT
\end{equation*}
変換$\hat{r}$を$r$の共役, $t$を共役変換と呼ぶ.
$\hat{r}$は回転を表わす変換であることに変わりはない.
$\hat{r}$は回転と共役であるともいう.
