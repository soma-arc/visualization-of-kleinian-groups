...
ここで議論されるフラクタルは数学的な研究対象にならないものが多いものの, アートと
しては非常に興味を引く形状を持つ.
...
シェーダでは, ピクセルごとにその座標とCPU側から渡される変数を受け取り，
これらを用いてピクセルの色を決定する.
アルゴリズム\ref{shaderSample}にシェーダの簡単な擬似コードを，
...
距離関数は任意の点が与えられたときに，図形からの最短距離を返す関数である．
...
\end{align*}
この関数は円周からの距離を返す.
平面上の各点に距離関数を作用させ,得られた距離が負であるときに色をつける
処理を書くと円を描くことができる．
複数の図形を描きたい時は，複数の距離関数を評価し，最も小さな距離を求めれ
ばよい.
距離関数による描画は，得られた距離によって色の濃さを変えるといった工夫で
図形をより滑らかに描くことができる．
アルゴリズム\ref{renderCircle}に円を描く疑似コードを，図
...
h\_doxasによるwgld\footnote{wgld: \url{https://wgld.org/d/glsl/}}には，シェー
...
の場合は難しい．
しかし，\textit{Distance Estimation}と呼ばれる手法を用いることで，
距離関数を陰関数から近似的に導出することができる.
フラクタルをシェーダを用いてレンダリングする際には，Distance Estimation
が重要となる．
...
導関数から得られる微分値にはヤコビアンが用いられたり, 中心差分法等
...
ことができる．
その他のEscape-time fractalには，\emph{ジュリア集合}(\textit{Julia set})(図
\ref{fig:julia})や\emph{ファトゥー集合}(\textit{Fatou set})等がある．
...
また, \textit{spherefold}は点が原点中心の球の内側にある時にその球に関する反転　を行う操作である．
