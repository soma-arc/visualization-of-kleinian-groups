...
る．
メビウス変換は一次分数変換とも呼ばれ，以下のように表される．
\begin{align*}
 a, b, c, d\in \hat{\mathbb{C}} \quad f(z) = \frac{az + b}{cz + d}
\end{align*}
メビウス変換は等角性や円円対応といった性質をもつ．
...
この図では変換$a$，$b$とそれらの逆変換である４種類の変換の合成の組合せを表
わしている．
例えば，$a$というラベルがふられたノードには外側へ３本のエッジが
...
図\ref{fig:schottky}では反転円の軌道を描いたが，同様にして異なる図形の
軌道を描くこともできる．
例えば，図\ref{fig:orbit}は中央に置いた星を４つの反転円による反転で移
した軌道を描いた．
図形の軌道も最終的に極限集合へと収束していくことがわかる．
...
ない．
%点の位置によって処理を切り替えることで，極限集合のエッジのみを描画する
%ことができる
...
まず，$T$を写像に与えられた点に作用させて$P$を得る．
そして，$P$を前述の平行移動と同様の操作で移し，点$P'$を得る．
...
そして，$T$を$C1, C2, C1'$に作用させて得られた円をそれぞれ$TC1, TC2,
TC1'$とよぶ．
