...
(原著:~\cite{MumfordSeriesWright200204})が適している．
インドラの真珠は数学者以外にもクライン群の魅力を伝えるために書かれた．
中には研究者レベルの高度な内容を扱う部分もあるが，高校卒業レベルの数学と
簡単なプログラムを組む能力があれば，自分で図をレンダリングしながら読み進
めることができる．
...
である．
共役を用いることで，ある変換をその性質を変えずに，異なる座標における変換
として置き換えることができる．
...
この合成変換は次の操作と等しい．
\begin{enumerate}
 \item 図\ref{fig:conju2}に示すように正方形の中心を$S^{-1}$で原点へと移す．
 \item 移された正方形に回転$T$を作用させる．
 \item 図\ref{fig:conju3}のように$S$で正方形を元の位置に戻す．これによっ
       て，$\hat{T}$で変換された正方形が得られる．
\end{enumerate}
...
例えば，ある変換$f(z)$，$g(z)$とそれらの逆変換という四つの変換を考える
...
例えば合成変換$f(f(g(f(z))))$は$aaba$という語で表される．
以下に二元生成群の生成元と，その構成要素のいくつかを例示した．
 \begin{align*}
  Generators =
   \begin{cases}
    a \colon f(z) \\
...
\emph{固定点}に移す．
変換$f(z)$の固定点とは$f(a) = a$となるような，変換の前後で動くことのない点
を指す．
...
点$a$を中心とする半径$r$の円に関する反転を$I(z)$とすると，式は次のようになる．
\begin{align*}
I(z) =
 \begin{cases}
...
できる．
図\ref{fig:stereoProject}に立体射影の模式図を示した．赤の円は単位円，
青の直線はX軸を表している．
...
この図法は縦軸に緯度，横軸に経度をとり，球面をそのまま写像する．
...
例えば，『インドラの真珠』では「おばあちゃんのレシピ」とよばれるレシピ
を主に扱う．
...
成することができる．
また，「おばあちゃんのレシピ」は群を可視化した際に綺麗な図が描画
されるように共役によって調整されている．
...
四つの円はそれぞれに円の反転の組み合わせが作用させられることにより，その
内側へと移されていく．
...
\begin{enumerate}
 ...
 \item それぞれの反転円に関する反転は図\ref{fig:level0inv}のように，自分以外の反
       転円を自身の内側に移す．
 \item この操作を四つの反転円で行なうと，それぞれの円の内側には三つの円が移
       され，図\ref{fig:level1}のように合わせて12個の小円ができる．
 \item 次に，新たにできた小円に対しても，その小円が属している反転円以外の反転を作
       用させる．図\ref{fig:level1inv}では，白く縁取りされた円に関する
       反転によって，その他の小円が移される様子を描いた．
 \item この操作も四つの反転円で行なうと，図\ref{fig:level2}のように小円の
       下に新たな小円ができる．
 \item このように，新たにできた小円に反転を作用させることを繰り返すと，
       最終的に図\ref{fig:levelMax}を得ることができる．
\end{enumerate}
...
極限集合に含まれる点はケーリーグラフにおいて無限の深さのノードが表す合成
変換に対応している．しかし，現実にそれを求めることは不可能である．そこで，
群の生成元の固定点をある程度の長さをもった変換の語で移すことで極限集合に
含まれる点である極限点を求める．
...
よばれるアルゴリズムを用いることで，固定点近辺の描画を効率化する．
...
図\ref{fig:iis-orb}では青い点に反転を繰り返し作用させた軌道を描いている．
...
また，反転円同士が接触しているとき，極限集合は平面を二つに分割する．
...
転，もしくは反転の合成を作用させる写像$G$を用いてこれを記述した．
...
移した軌道を描画する．
図\ref{fig:3baseSphereGen}における灰色の球を\emph{反転球}，緑色
の球を\emph{基本球}とよぶ．
図\ref{fig:3baseSphereGen}に描かれている三つの基本球を，すべての反転球の反
転の組み合わせで移した軌道が図\ref{fig:3baseSphereOrb}のようになる．
...
実際の距離関数ではレイの先端にIISを作用させる過程で，球の反転を行う毎に，
そのヤコビアンを累積させていく．
...
そのため，距離関数は実際の最短距離よりも大きな距離を返してしまい，
レイはオブジェクトを突きぬけてしまう．
...
きさにしたときのものである．
球の上に極限集合をみることができる．
図\ref{fig:limitObj}はそこから基本球の大きさを，さらに大きくした図である．
...
四元数行列で表現する．四元数は四つの要素をもつ数であり，メビウス変換のパ
ラメータの数は非常に多くなる．
...
2017}\footnote{BRIDGES WATERLOO 2017 Mathematics, Music, Art,
Architecture, Education, Culture:\\
 \url{http://bridgesmathart.org/bridges-2017/}}に投稿中である．
...
$P$は制御点である．この点に$C1$の反転を作用させて得られた点を$P'$，$C2$の
反転を作用させて得られた点を$P''$とすると，$C3$は$P,~P',~P''$の三点から
定義される．
...
二次元の場合と同様に，単体の球の反転はメビウス変換ではない．
二つの球による反転の組がメビウス変換である．
...
\end{cases}
\end{align*}
基本領域は赤い球の外側かつ青い球の内側である．つまり，$S1$の外側かつ
$S1'$の内側の領域である．
...
平行な二直線の一方をX軸に垂直に，もう一方の直線をY軸に沿うように変換する
共役変換$T$を求める．
...
に移すことができる．
また，$i$から，点を基本領域へと移すまでの通常の反転の回数を求めることが
できる．
...
最適化された写像の操作過程は次のようになる．
まず，写像に与えられた点${P}$に$T$を作用させ，点$T(P)$を得る．
...
できる．また，$i$から，$P$が基本領域へと移るまでの通常の反転の回数を求め
ることができる．
...
擬フックス群は\emph{円辺形}を囲む円の組から生成元を得ることができる．
図\ref{fig:schottky}において，中央の円盤に囲まれた基本領域が円辺形で
ある．
...
擬フックス群の極限集合は円や球の和集合となる．
阿原・荒木による動画\footnote{Quasi-fuchsian fractals:~
\url{https://www.youtube.com/watch?v=3lcO9zRCv-4}}ではこのフラクタルの構
成過程をみることができる．
図\ref{fig:quasiFuchsian}は球の座標と半径を計算することで描画したが，
レイマーチングとDistance Estimationによる高速描画のアルゴリズムが
...
しかし，高次元のクライン群には未だに解明されていないことが多い．
...
Aaron MontagはIterated Function Systemを用いて極限集合を描画する方法を
